...
if [ ! -f "/db-init/done" ]; then
  ...
  echo 'user = Decidim::User.where(email: "decidim@puzzle.ch").first; user.invite!; "*************************************************** ACTIVATE YOUR ACCOUNT **************************************************** #{user.email} on http://localhost:3000/users/invitation/accept?invitation_token=#{user.raw_invitation_token}"' | bundle exec rails console
  date > /db-init/done
else
